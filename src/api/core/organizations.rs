...
async fn create_organization(headers: Headers, data: JsonUpcase<OrgData>, mut conn: DbConn) -> JsonResult {
    ...
    if OrgPolicy::is_applicable_to_user(&headers.user.uuid, OrgPolicyType::SingleOrg, &mut conn).await {
        err!(
    ...
    match UserOrganization::find_by_user_and_org(&headers.user.uuid, &org_id, &mut conn).await {
        ...
        Some(user_org) => {
            if user_org.atype == UserOrgType::Owner {
                let num_owners =
                    UserOrganization::find_by_org_and_type(&org_id, UserOrgType::Owner as i32, &mut conn).await.len();
    ...
    match User::find_by_mail(&claims.email, &mut conn).await {
        Some(_) => {
            ...
            if let (Some(user_org), Some(org)) = (&claims.user_org_id, &claims.org_id) {
                ...
                let user_twofactor_disabled = TwoFactor::find_by_user(&user_org.user_uuid, &mut conn).await.is_empty();

                let policy = OrgPolicyType::TwoFactorAuthentication as i32;
                let org_twofactor_policy_enabled =
                    match OrgPolicy::find_by_org_and_type(&user_org.org_uuid, policy, &mut conn).await {
                        Some(p) => p.enabled,
                        None => false,
                    };
                ...
                let single_org_policy_enabled = match OrgPolicy::find_by_org_and_type(
                    &user_org.org_uuid,
                    OrgPolicyType::SingleOrg as i32,
                    &mut conn,
                )
                .await
                {
                ...
                if single_org_policy_enabled && user_org.atype < UserOrgType::Admin {
                    let is_member_of_another_org =
                        UserOrganization::find_any_state_by_user(&user_org.user_uuid, &mut conn)
                            .await
                            .into_iter()
                ...
                if OrgPolicy::is_applicable_to_user(&user_org.user_uuid, OrgPolicyType::SingleOrg, &mut conn).await {
                    err!(
...
async fn get_policy(org_id: String, pol_type: i32, _headers: AdminHeaders, mut conn: DbConn) -> JsonResult {
    ...
    let policy = match OrgPolicy::find_by_org_and_type(&org_id, pol_type, &mut conn).await {
        Some(p) => p,
...
async fn put_policy(
    ...
    if pol_type_enum == OrgPolicyType::SingleOrg && data.enabled {
        for member in UserOrganization::find_by_org(&org_id, &mut conn).await.into_iter() {
            ...
            if member.atype < UserOrgType::Admin && member.status != UserOrgStatus::Invited as i32 {
                let is_member_of_another_org = UserOrganization::find_any_state_by_user(&member.user_uuid, &mut conn)
                    .await
                    .into_iter()
    ...
    let mut policy = match OrgPolicy::find_by_org_and_type(&org_id, pol_type, &mut conn).await {
        Some(p) => p,
        None => OrgPolicy::new(org_id, pol_type_enum, "{}".to_string()),
    };
...
async fn import(org_id: String, data: JsonUpcase<OrgImportData>, headers: Headers, mut conn: DbConn) -> EmptyResult {
    ...
    if data.OverwriteExisting {
        for user_org in UserOrganization::find_by_org_and_type(&org_id, UserOrgType::User as i32, &mut conn).await {
